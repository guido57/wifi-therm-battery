...
void EspNow::Loop(){ 
  ...
  if(waiting_messages>0){
    ...
    else if( 0 == strcmp(sm.key, "mqtt_user")){ 
      strcpy(settings.entries.MQTTUser, sm.value);  
      settings.Save();
      strcpy(sm_ack.key,"mqtt_user");
      Send(mac,WiFi.channel(),sm_ack);
    }
    
    else if( 0 == strcmp(sm.key, "mqtt_pwd")){ 
      strcpy(settings.entries.MQTTPassword, sm.value);  
      settings.Save();
      strcpy(sm_ack.key,"mqtt_pwd");
      Send(mac,WiFi.channel(),sm_ack);
    }
